...
type ReaderState = (String, String, Int, Int, Int) -- out, close, (, [, {
...
advanceHead (x:xs) = do
    ...
    if | cl == "" -> case x of
            ...
            '[' -> put (out ++ [x] ++ "\n", "]", cp, cb, cc)
            '{' -> put (out ++ [x] ++ "\n", "}", cp, cb, cc)
            otherwise -> put (out ++ [x], cl, cp, cb, cc)
       | x == ')' && cl == ")" && cp == 0 ->
            put (out ++ "\n" ++ [x], cl, cp - 1, cb, cc)
       | x == ']' && cl == "]" && cb == 0 ->
            put (out ++ "\n" ++ [x], cl, cp, cb - 1, cc) 
       | x == '}' && cl == "}" && cc == 0 ->
            put (out ++ "\n" ++ [x], cl, cp, cb, cc - 1)  
       | otherwise -> case x of
            '(' -> put (out ++ [x], cl, cp + 1, cb, cc)
            '[' -> put (out ++ [x], cl, cp, cb + 1, cc)
            '{' -> put (out ++ [x], cl, cp, cb, cc + 1)
            ')' -> put (out ++ [x], cl, cp - 1, cb, cc)
            ']' -> put (out ++ [x], cl, cp, cb - 1, cc)
            '}' -> put (out ++ [x], cl, cp, cb, cc - 1)
            ',' -> if cp == 0 && cb == 0 && cc == 0
                     then put (out ++ [x, '\n'], cl, cp, cb, cc)
                     else put (out ++ [x], cl, cp, cb, cc)
            otherwise -> put (out ++ [x], cl, cp, cb, cc)       
    advanceHead xs 
